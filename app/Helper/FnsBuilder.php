<?php
//phpcs:disable
namespace RT\BlencoCore\Helper;
...
class FnsBuilder {
	...
	public static function get_builder_type() {
		...
		$allPost = [
			'post' => __( 'Post', 'blenco-core' ),
			'page' => __( 'Post', 'blenco-core' ),
		];
		...
		$default_pages = [
			'is_front_page' => __( 'Front Page', 'blenco-core' ),
			'is_home'       => __( 'Blog / Posts Page', 'blenco-core' ),
			'is_search'     => __( 'Search Page', 'blenco-core' ),
			'is_archive'    => __( 'Archive', 'blenco-core' ),
			'is_404'        => __( '404 Page', 'blenco-core' ),
		];

		if ( class_exists( 'WooCommerce' ) ) {
			$default_pages['is_shop'] = __( 'WooCommerce Shop Page', 'blenco-core' );
		}

		$selection_options = [
			'sitewide' => [
				'label' => __( 'Sitewide', 'blenco-core' ),
				'value' => [
					'sitewide-global'    => __( 'Entire Website', 'blenco-core' ),
					'sitewide-singulars' => __( 'All Singulars', 'blenco-core' ),
					'sitewide-archives'  => __( 'All Archives', 'blenco-core' ),
				],
			],

			'default-pages' => [
				'label' => __( 'Default Pages', 'blenco-core' ),
				'value' => $default_pages,
			],
		];
		...
		// 	$pTypeVal = [];
		// 	if ( $post_type == 'page' ) {
		// 		$pTypeVal['single|page'] = __( "All Pages", "blenco-core" );
		// 	} else {
		// 		$pTypeVal = [
		// 			"single-$post_type"  => sprintf( __( 'All %s Single', 'blenco-core' ), $post_type_name ),
		// 			"archive-$post_type" => sprintf( __( 'All %s Archive', 'blenco-core' ), $post_type_name ),
		// 		];
		// 	}
		...
		foreach ( $allPost as $post_type => $post_type_name ) {
			...
			if ( 'page' === $post_type ) {
				...
				// translators: %s: Post type name (e.g. Blog, Portfolio)
				$single_label = __( 'All %s Single', 'blenco-core' );

				// translators: %s: Post type name (e.g. Blog, Portfolio)
				$archive_label = __( 'All %s Archive', 'blenco-core' );

				$pTypeVal = [
					"single-$post_type"  => sprintf( $single_label, $post_type_name ),
					"archive-$post_type" => sprintf( $archive_label, $post_type_name ),
				];
			}
			...
			if ( ! empty( $taxonomies ) ) {
				foreach ( $taxonomies as $taxonomy ) {
					...
					if ( in_array( $taxonomy->name, [ 'post_format' ], true ) ) {
						continue;
					}

					$pTypeVal[ 'tax-' . $taxonomy->name ] = esc_html( $taxonomy->label );
				}
			}
		...
		$selection_options['custom'] = [
			'label' => __( 'Custom Page / Post', 'blenco-core' ),
			'value' => [
				'custom' => __( 'Choose custom page / post', 'blenco-core' )
			]
		];*/

		return apply_filters( 'blenco_builder_type', $selection_options );
