	...
	<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
		...
		<?php if ( $login == 'yes' ) { ?>
			<li class="rt-user-login rt-button">
				<a class="btn" href="<?php echo esc_url( wp_login_url() ) ?>" aria-label="user login">
		...
		<?php if ( $hamburger_2 == 'yes' ) { ?>
			<?php blenco_hanburger2( 'desktop-hamburg-2' ); ?>
			<?php blenco_hanburger2( 'mobile-hamburg' ); ?>
		<?php } ?>
