        ...
        <div class="swiper-wrapper <?php if( $slider_animation == 'yes' ) { ?>animation<?php } ?>">
            ...
            foreach ($banners as $banner){ ?>
                <div class="swiper-slide single-slide slide-<?php echo esc_attr( $i ); ?>">
                    <div class="single-slider" data-background="<?php echo esc_attr($banner['img']); ?>">
                        <div class="container">
                            <div class="content-wrap">
                                <div class="slider-content">
                                    <?php if( !empty( $banner['sub_title'] ) ) { ?>
                                        <div class="sub-title"><?php echo blenco_html( $banner['sub_title'], 'allow_title' );?></div>
                                    <?php } if( !empty( $banner['title'] ) ) { ?>
                                        <<?php echo esc_attr( $title_tag ) ?> class="slider-title"><?php echo blenco_html( $banner['title'], 'allow_title' );?></<?php echo esc_attr( $title_tag ) ?>>
                                    <?php } if( !empty( $banner['content'] ) ) { ?>
                                        <div class="slider-text"><?php echo blenco_html( $banner['content'], 'allow_title' );?></div>
                                    <?php } ?>
                                    <?php if( !empty( $banner['button_text'] ) ) { ?>
                                        <div class="slider-btn-area rt-button">
                                            <a class="btn" href="<?php echo esc_url( $banner['button_url'] ); ?>">
                                                <span class="btn-icon">
                                                    <i class="icon-rt-arrow-right-1"></i>
                                                </span>
											    <span class="btn-text"><?php echo blenco_html( $banner['button_text'], 'allow_title' );?></span>
										    </a>
                                        </div>
                                    <?php } ?>
                                </div>
